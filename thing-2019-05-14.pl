...
sub doStack {
    ...
    $to = "agent";
    my $data_to_json = { to => $nom_from, from => $to, subject => $subject };
...
#    my $tasks  = $client->new_task_set;
    my $handle = $client->do_task(
        call_agent => $datagram,
        {
           on_complete => sub { 
		#$json_response = $handle;	
                #print Dumper \$_;
		$json_response = ${ $_[0] }, "\n" 
	    }
        }
    );
#               $json_response = $ {$handle[0] }, "\n";
...
sub doSubject {
    ...
    foreach my $line (@output) {
        ...
        if ($flag) {
            $subject = $line;
	    $subject =~ s/\r//g; # strip carriage return at end	
            last;
        }
